...
function convert(data, config) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var datapath = slash(data.path);
    var cachedir = slash(config.org.cachedir) || "";
    var outputfile = slash(tmpobj.name);
    var debugfile = slash(dbgobj.name);
    //var debugfile = "C:/projects/hexo-renderer-org/aa";
    var theme = config.org.theme || "";

    console.log("File: ", data.path);
    console.log("File2: ", slash(data.path));

    var emacs_lisp = `
    ;; Execute our exporter code
    (let ((args '(:file         "${datapath}"
                  :cache-dir    "${cachedir}"
...
//    emacs_lisp = emacs_lisp.replace(/\t/g, "");
...
//    var exec_args = ['-Q', '-nw', '--eval', emacs_lisp];
    ...
    proc.on('data', function(d) {
      ...
      //   var aa = fs.readFileSync(dbgobj.name, 'utf8');
      if (fs.existsSync(debugfile)) {
        var aa = fs.readFileSync(debugfile, 'utf8');
        console.log("on data: ", aa);
      }
